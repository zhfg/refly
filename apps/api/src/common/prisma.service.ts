...
export class PrismaService extends PrismaClient implements OnModuleInit {
  ...
  constructor(@InjectQueue(QUEUE_EVENT) private queue: Queue<ReportEventData>) {
    super({
      log: [
        {
          emit: 'event',
  ...
  registerClientExtensions() {
    const sendCollectionCreateEvent = async (coll: Pick<Collection, 'uid' | 'collectionId'>) => {
      try {
        await this.queue.add(CHANNEL_REPORT_EVENT, {
          uid: coll.uid,
          eventType: 'create',
    ...
    const sendCollectionUpdateEvent = async (coll: {
      ...
      try {
        await this.queue.add(CHANNEL_REPORT_EVENT, {
          ...
          entityType: 'collection',
          entityId:
            typeof coll.collectionId === 'string' ? coll.collectionId : coll.collectionId.set,
        });
      } catch (e) {
        console.error(e); // TODO: use service logger (but how?)
      }
      ...
      query: {
        collection: {
          ...
          async upsert({ args, query }) {
            ...
            if (
              new Date(res['createdAt'] as string).getTime() === args.create.createdAt.getTime()
            ) {
...
export const PrismaProvider: Provider = {
  ...
  useFactory: (queue: Queue<ReportEventData>) => {
    return new PrismaService(queue).registerClientExtensions();
  },
