...
export const SelectedContentList = (props: SelectedContentListProps) => {
  ...
  const { handleExit, handleRemoveAll, handleResetState, handleRemoveMark } = useSelectedMark();
      ...
      <div className="selected-content-list-container">
        {marks.map((item, index) => (
          <Card
            key={index}
            style={{ width: '100%' }}
            extra={<IconClose className="selected-content-item-action" onClick={() => handleRemoveMark(item?.xPath)} />}
          >
