...
export const ThreadItem = (props: ThreadItemProps) => {
  ...
  const getInputText = () => {
    ...
    if (showSelectedMarks)
      return t(
        "threadDetail.item.input.searchPlaceholder.currentSelectedContent"
      );
    if (searchTarget === SearchTarget.SelectedPages)
      return t("threadDetail.item.input.searchPlaceholder.selectedWeblink");
    if (searchTarget === SearchTarget.CurrentPage)
      return t("threadDetail.item.input.searchPlaceholder.current");
    if (searchTarget === SearchTarget.SearchEnhance)
      return t("threadDetail.item.input.searchPlaceholder.internet");
    if (searchTarget === SearchTarget.All)
      return t("threadDetail.item.input.searchPlaceholder.all");
  };
  ...
  const handleAskFollowing = async (question?: string) => {
    ...
    if (!newQAText) {
      message.info(t("threadDetail.item.input.status.emptyNotify"));
      return;
    }
