    ...
    () => ({
      tools: [
        ...
        },
        {
          ...
          domain: "launchpad",
          tooltip: t(
            `canvas.toolbar.${showLaunchpad ? "hideLaunchpad" : "showLaunchpad"}`
          ),
          hoverContent: {
  ...
  ({
    ...
    const button = (
      <Button
        ...
        `}
        icon={
          <tool.icon className="h-[18px] w-[18px] text-gray-600 group-hover:text-gray-900" />
        }
      />
    );
...
export const CanvasToolbar = memo<ToolbarProps>(({ onToolSelect }) => {
  ...
  const { showLaunchpad, setShowLaunchpad, showEdges } = useCanvasStoreShallow(
    (state) => ({
      showLaunchpad: state.showLaunchpad,
  ...
  const { setImportResourceModalVisible } = useImportResourceStoreShallow(
    (state) => ({
      setImportResourceModalVisible: state.setImportResourceModalVisible,
    })
  );

  const contextItems = useContextPanelStoreShallow(
