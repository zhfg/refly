...
export const useNavigationContextStore = create<NavigationContextState>()(
  devtools((set) => ({
    ...
    setNavigationContext: (val: NavigationContext) => set((state) => ({ ...state, navigationContext: val })),
  })),
);

export const useNavigationContextStoreShallow = <T>(selector: (state: NavigationContextState) => T) => {
