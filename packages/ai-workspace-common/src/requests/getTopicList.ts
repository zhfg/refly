...
import { Topic, ListPageProps } from '@refly-packages/ai-workspace-common/types';
...
const handler = async (
  ...
  try {
    const [err, topicRes] = await request<{ list: Topic[]; total: number }>(appConfig.url.getTopicList, {
      method: 'GET',
      body: req.body,
    });
    if (err) {
