...
export class BasicSummaryWithNoteSkill extends BaseSkill {
  name = 'basic_summary_with_note';
  displayName = {
    en: 'Basic Summary with Note',
    'zh-CN': '基于选中笔记的总结',
  };

  invocationConfig: SkillInvocationConfig = {
    inputRules: [{ key: 'query' }],
    contextRules: [{ key: 'noteIds' }],
  };

  description = 'Give a summary of the selected note';
  ...
  async generate(state: GraphState, config?: SkillRunnableConfig) {
    ...
    // 1.1 contentList
    const contentListText = lastNote?.data?.content;
