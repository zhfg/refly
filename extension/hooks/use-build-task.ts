...
import { defaultMessageState, useMessageStateStore } from '../stores/message-state';
...
export const useBuildTask = () => {
    ...
    const handleStreamingMessage = (msg) => {
            ...
            if (
               ...
               currentMessageState.pendingMsg?.length === 0
                ...
                if (currentMessageState.taskType === TASK_TYPE.CHAT) {
                    ...
                    currentMessageState.taskType === TASK_TYPE.QUICK_ACTION
                ) {
        ...
        if ( currentMessageState.taskType === TASK_TYPE.CHAT) {
            if ( currentMessageState.pendingFirstToken) {
                ...
                chatStore.setMessages([...currentChatState.messages, { ...lastReplyMessage }])
    ...
    const unbindExtensionPorts = () => {
        ...
        genResponsePortRef.current?.onMessage?.removeListener?.(handleStreamingMessage)
