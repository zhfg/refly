...
import { ACTION_TYPE, TASK_TYPE, type Mode, type QUICK_ACTION } from "~/types"
// stores
...
export const PopupContent = (props: PopupContentProps) => {
  ...
  const { buildGenTitleTaskAndGenResponse, buildShutdownTaskAndGenResponse } =
    useBuildTask()
      ...
      <header>
        <div className="header-left">
          ...
          <span className="text-color">
            {quickActionStore.currentMode?.text}
          </span>
          ...
          <div className="result">
            ...
            {messageStateStore.taskType === TASK_TYPE.QUICK_ACTION &&
              messageStateStore?.pending && (
                <Spin size={14} className="loading" />
              )}
          </div>
          <div>
            {messageStateStore.taskType === TASK_TYPE.QUICK_ACTION &&
              ...
              <>
                ...
                <IconTip text="重试" position="bottom">
                  <IconRefresh
                    className="user-action-btn"
                    onClick={(_) =>
                      props.handleQuickActionGenResponse(
